...
void SendMessage(const std::string& message) {
        int sock = m_socket;
        if (sock == -1) {
            std::cerr << "IPCClient: Socket desconectado — tentando reconectar...\n";
            std::this_thread::sleep_for(std::chrono::milliseconds(500));
            sock = m_socket;
        }
        if (sock == -1) {
            std::cerr << "IPCClient: Socket ainda desconectado — mensagem descartada\n";
            return;
        }
        std::string raw = message + "\n";
        ssize_t sent = write(sock, raw.c_str(), raw.size());
        if (sent < 0) {
            std::cerr << "IPCClient: Falha ao escrever no socket (errno=" << errno << ")\n";
            m_socket = -1;
            close(sock);
        }
    }
  ...
  void PollInbound() {
    ...
    for (const auto& [cb, resp] : batch) {
      if (cb) {
        std::cerr << "[Frontend] Executando callback reqID=" << resp.requestId << " type=" << resp.type
                  << " status=" << resp.status << "\n";
        cb(resp);
      }
    }
  ...
  void HandleRawFrame(const std::string& msg) {
    ...
    if (respOpt.has_value()) {
      ...
      if (cb) {
        ...
        std::cerr << "[Frontend] Resposta RPC recebida reqID=" << resp.requestId << " type=" << resp.type
                  << " status=" << resp.status << "\n";
        return;
      } else {
        std::cerr << "[Frontend] Resposta sem callback pendente reqID=" << resp.requestId << " type=" << resp.type
                  << " status=" << resp.status << "\n";
      }
    }
