...
public:
    ...
    explicit RenderContext() {
        m_scissorStack.reserve(kScissorStackCapacity);
    }
    ...
    void DrawRect(Rect r, Color c) const noexcept {
        DrawRectangleRec(ToRL(r), ToRL(c));
    }
    ...
    void DrawText(std::string_view text, Vec2 pos,
                  float fontSize, Color c,
                  float spacing = 0.5f) const noexcept {
    ...
    void PushScissor(Rect r) noexcept {
        ...
        BeginScissorMode(static_cast<int>(r.x),
                         static_cast<int>(r.y),
    ...
    [[nodiscard]] bool HasActiveScissor() const noexcept {
...
private:
    ...
    [[nodiscard]] static ::Rectangle ToRL(Rect r) noexcept {
        return {r.x, r.y, r.width, r.height};
    }

    [[nodiscard]] static ::Color ToRL(Color c) noexcept {
