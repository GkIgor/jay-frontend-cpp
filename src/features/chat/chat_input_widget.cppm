...
public:
  ...
  void Init() override {
    m_sendButton = std::make_unique<jay::shared::widgets::Button>("ENVIAR", [this]() { SubmitText(); });
    m_sendButton->Init();
  }
  ...
  void Render(jay::engine::RenderContext& ctx) const override {
    ...
    jay::engine::Rect textClipRect{m_bounds.x + 12.0f, m_bounds.y + 6.0f, m_editAreaW, m_bounds.height - 12.0f};
    ...
    bool focused = m_isFocused;

    if (m_editor.GetTextLength() == 0) {
      ctx.DrawText("Digite uma mensagem...", {startX, startY}, 15.0f, jay::engine::Color{139, 148, 158, 255});

      if (focused && std::fmod(m_caretAnimTime, 1.0f) < 0.5f) {
        ctx.DrawRect(jay::engine::Rect{startX, startY, 2.0f, stepY}, jay::engine::Color{88, 166, 255, 255});
      }
    } else {
        ...
        for (size_t l = 0; l < physicalLines.size(); ++l) {
          ...
          for (size_t i = 0; i < cps.size(); ++i) {
            if (charAbsIdx >= selStart && charAbsIdx < selEnd) {
              ...
              ctx.DrawRect(jay::engine::Rect{startX + x1, lineY, charW, stepY}, jay::engine::Color{31, 111, 235, 120});
            }
            charAbsIdx++;
          }
      ...
      if (std::fmod(m_caretAnimTime, 1.0f) < 0.5f) {
        ...
        ctx.DrawRect(jay::engine::Rect{caretX, caretY, 2.0f, stepY}, jay::engine::Color{88, 166, 255, 255});
      }
    }
...
private:
  ...
  void UpdateSendButtonBounds() {
    ...
    m_sendButton->SetBounds(
      jay::engine::Rect{m_bounds.x + m_bounds.width - btnW - 6.0f, m_bounds.y + 6.0f, btnW, btnH});
  }
